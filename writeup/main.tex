...
transformation looks like. Fourteen preregistrations, $335$ tests, seven of our own checkers caught
...
anywhere, and erased $k$ fitted directions as a subspace rather than one direction. Its primary
question was unanswerable: a probe refit on the erased activations still separated the classes at
$\mathrm{cv}=1.000$ at every $k$ up to $56$ with $n=60$, so the erasure gate failed and no
re-encoding claim is made. What it does establish is about the \emph{operation}. Removing one fitted
direction removed $40\%$ of a comparable layer-32 signal and eight removed $69\%$, plateauing near
$30\%$ thereafter, while a random subspace of the same rank removed essentially nothing. So the
single-direction projection used here is a small fraction of a full erasure and never makes the
property undecodable. \textbf{The honest reading of this section is therefore narrower than
``the state outlives its own cause'': $86\%$ of the effect survives an operation that removes one
direction and demonstrably does not remove the property.} The direct test would be to re-run this
arm at $k>1$, which we have not done.
...
All fourteen preregistrations, every raw artifact committed unscored, every analyzer, and $335$ tests
