...
Code, data, all fifteen preregistrations and every raw artifact: \url{\repourl}.
...
transformation looks like. Fifteen preregistrations, $354$ tests, nine of our own checkers caught
...
\paragraph{How weak is ``erase''? Weaker than the word suggests, and we needed three attempts to
measure it.} Two further preregistered arms induced a valence state \emph{by prompt}, with no
injection anywhere, and erased it at layer $E$ while reading an untouched probe at layer 32. The
first used iterative nullspace projection at $n{=}60$ and $n{=}1800$; the second used LEACE
\citep{belrose2023}, whose erasure is rank one for a binary label and which guarantees the
class-conditional means coincide. \textbf{Both erasure gates were broken, in two different ways, and
both are recorded in \S\ref{sec:checkers}.} The measurement that survives all three attempts is the
layer-32 comparison, which never depended on the gate.

\textbf{What survives, and it is a specificity result.} On held-out topics with the eraser fit on
disjoint topics, a \textbf{rank-one} LEACE eraser removes $91$--$95\%$ of the layer-32 separation at
$E{=}30$ and $68$--$70\%$ at $E{=}26$, while a \textbf{rank-matched random} eraser removes none of
it, leaving $100\%$ intact on both models. One correctly chosen dimension does almost all of the
work; a randomly chosen one of the same rank does none. The reduction is about \emph{which}
direction was removed, not about removing a dimension.

\textbf{What that does to this section.} Removing one fitted direction is a targeted operation, not
a small one, so the earlier reading of $86\%$ survival as ``the state outlives its own cause'' is
still too strong: the honest statement is that $86\%$ of the effect survives removal of one
direction, and we have not established that removing that direction removes the property. The
erasure gate that would decide it has failed three times, most recently because a train-fit eraser
does not fully transfer to held-out topics, which is a real limit rather than a bug. We report the
survival and decline the interpretation.
...
All fifteen preregistrations, every raw artifact committed unscored, every analyzer, and $354$ tests
